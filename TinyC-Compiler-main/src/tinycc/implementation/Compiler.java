...
public class Compiler {
	...
	private ASTfactoryImpl ASTfactoryImpl;  
	...
	public Compiler(final Diagnostic diagnostic) {
		...
		this.ASTfactoryImpl = new ASTfactoryImpl();
	}
    ...
    public void checkSemantics() {
		SemanticAnalyzer analyzer = new SemanticAnalyzer(diagnostic);
		

		for (ExternalDeclaration external : ASTfactoryImpl.getExternalDeclarations()) {
			analyzer.declareVariable(external.getName(), external.getType());
		}
		for (FunctionDefinition function : ASTfactoryImpl.getFunctionDefinitions()) {
			analyzer.declareFunction(function.getName(), function.getType());
		}
	...
	public void generateCode(final MipsAsmGen out) {
		CodeGenerator codeGen = new CodeGenerator(out);
        codeGen.generateGlobalVariables(ASTfactoryImpl.getExternalDeclarations());
        codeGen.generateFunctions(ASTfactoryImpl.getFunctionDefinitions());	
	}
